...
We presented a study of certified and sampling-based uncertainty quantification for matmul-free language models. Our first key finding is that certified IBP bounds explode due to RMSNorm---a data-dependent division operation that is fundamentally incompatible with interval arithmetic. When bounds are wide enough that dimensions cross zero, the RMSNorm denominator collapses toward $\sqrt{\epsilon}$, producing output bounds that are up to 500,000$\times$ wider than those obtained via linear relaxation. BitLinear layers, which use only ternary accumulation, are well-behaved under IBP; it is the normalization and multiplicative gating that cause the failure.

Second, we demonstrated that CROWN linear relaxation provides a practical solution. Our diagonal-Jacobian CROWN implementation keeps RMSNorm bounds stable at $\sim$10 regardless of the input bound range, compared to certified IBP which scales linearly. This represents a tightening ratio of $5\times$ to $5\times 10^5$ depending on the bound width. A full IBP-CROWN implementation---using IBP for efficient intermediate bounds and CROWN for tight final bounds through the normalization layers---is the natural next step toward practical certified UQ for recurrent LLMs.

Third, Monte Carlo sampling provides stable, model-agnostic uncertainty estimates with bound widths of 0.5--10 across $\varepsilon \in [0.001, 0.1]$. Comparing MMfreeLM-370M against GPT-2 Medium (355M), we found that the matmul-free architecture does not fundamentally alter embedding-level robustness: at $\varepsilon{=}0.01$, MC bound widths are 0.89 vs.\ 1.98, with GPT-2 showing higher top-1 stability (0.99 vs.\ 0.59) despite wider bounds. The two-pass corner evaluation systematically underestimates uncertainty by 2--6$\times$ compared to MC, confirming that output extrema do not occur at input extrema for non-monotonic architectures.
